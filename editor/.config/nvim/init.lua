...
-- [[ GoLand-matching keymaps ]]
-- escape
vim.keymap.set('i', 'jk', '<Esc>')

-- clear search highlight (alternative to <Esc>)
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', 'ga', '<cmd>Telescope<CR>', { desc = 'search everything' })

      -- GoLand-matching LSP keymaps via Telescope (overrides Kickstart gr* defaults)
      vim.api.nvim_create_autocmd('LspAttach', {
        group = vim.api.nvim_create_augroup('telescope-lsp-attach', { clear = true }),
        callback = function(event)
          local buf = event.buf

          -- goto definition (GoLand: gd)
          vim.keymap.set('n', 'gd', builtin.lsp_definitions, { buffer = buf, desc = 'goto definition' })

          -- goto type definition (GoLand: gD)
          vim.keymap.set('n', 'gD', builtin.lsp_type_definitions, { buffer = buf, desc = 'goto type definition' })

          -- goto implementation (GoLand: gi)
          vim.keymap.set('n', 'gi', builtin.lsp_implementations, { buffer = buf, desc = 'goto implementation' })

          -- show usages / references (GoLand: gr)
          vim.keymap.set('n', 'gr', builtin.lsp_references, { buffer = buf, desc = 'show usages' })

          -- file structure popup (GoLand: <leader>fs)
          vim.keymap.set('n', '<leader>fs', builtin.lsp_document_symbols, { buffer = buf, desc = 'file structure' })

          -- workspace symbols (GoLand: gs)
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- GoLand-matching LSP keymaps
          map('go', vim.lsp.buf.declaration, 'goto declaration')
          map('K', vim.lsp.buf.hover, 'hover docs')
          map('<leader>qf', vim.lsp.buf.code_action, 'code action', { 'n', 'x' })
          map('<leader>rn', vim.lsp.buf.rename, 'rename')
          map('<leader>se', vim.diagnostic.open_float, 'show error')
  ...
  { -- Autoformat
    ...
    keys = {
      {
        -- GoLand: <leader>fc for ReformatCode
        '<leader>fc',
  ...
  -- Comment toggle (GoLand: <leader>c)
  {
  ...
  { -- Highlight, edit, and navigate code
    ...
    config = function()
      ...
      local parsers = { 'bash', 'c', 'cpp', 'diff', 'go', 'gomod', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'rust', 'typescript', 'vim', 'vimdoc' }
